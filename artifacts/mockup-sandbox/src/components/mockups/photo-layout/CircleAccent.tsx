...
export function CircleAccent() {
  return (
    <div style={{ fontFamily: "'DM Sans', sans-serif", background: 'linear-gradient(135deg, #18131f 0%, #22192e 42%, #2b1f39 100%)', minHeight: '100vh', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ maxWidth: '1180px', margin: '0 auto', padding: '120px 48px 80px', display: 'grid', gridTemplateColumns: '1fr clamp(260px, 28vw, 380px)', gap: 'clamp(32px, 6vw, 88px)', alignItems: 'center', minHeight: '100vh', position: 'relative', zIndex: 1 }}>
        <div>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
          <div style={{ position: 'relative' }}>
            <div style={{ position: 'absolute', inset: '-16px', borderRadius: '50%', border: '1px solid rgba(198,155,99,0.22)', pointerEvents: 'none' }} />
            <div style={{ position: 'absolute', inset: '-32px', borderRadius: '50%', border: '1px solid rgba(198,155,99,0.10)', pointerEvents: 'none' }} />
            <div style={{ position: 'absolute', inset: 0, borderRadius: '50%', background: 'radial-gradient(circle at 30% 30%, rgba(198,155,99,0.18) 0%, transparent 60%)', pointerEvents: 'none' }} />
            <img
              src={PHOTO_URL}
              alt="Kim Hoggan"
              style={{ width: 'clamp(220px, 26vw, 340px)', height: 'clamp(220px, 26vw, 340px)', objectFit: 'cover', objectPosition: 'center top', borderRadius: '50%', border: '3px solid rgba(198,155,99,0.40)', boxShadow: '0 32px 80px rgba(10,8,15,0.50), 0 0 0 1px rgba(235,200,152,0.08)', display: 'block' }}
            />
          </div>
        </div>
      </div>
