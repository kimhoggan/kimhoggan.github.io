...
export function LargeLeft() {
  return (
    <div style={{ fontFamily: "'DM Sans', sans-serif", background: 'linear-gradient(135deg, #18131f 0%, #22192e 42%, #2b1f39 100%)', minHeight: '100vh', position: 'relative', overflow: 'hidden' }}>
      <style>{`
        .ll-grid { display: grid; grid-template-columns: 340px 1fr; min-height: 100vh; }
        .ll-photo { position: relative; overflow: hidden; min-height: 340px; }
        .ll-photo img { width: 100%; height: 100%; object-fit: cover; object-position: center top; display: block; }
        .ll-fade-right { position: absolute; inset: 0; background: linear-gradient(to right, transparent 60%, rgba(26,21,35,0.85) 100%); }
        .ll-fade-bottom { position: absolute; inset: 0; background: linear-gradient(to top, rgba(26,21,35,0.45) 0%, transparent 40%); }
        .ll-content { display: flex; align-items: center; padding: 80px 72px 80px 56px; }
        @media (max-width: 768px) {
          .ll-grid { grid-template-columns: 1fr; grid-template-rows: 280px 1fr; }
          .ll-photo { height: 280px; }
          .ll-fade-right { background: linear-gradient(to bottom, transparent 50%, rgba(26,21,35,0.90) 100%); }
          .ll-content { padding: 40px 28px 60px; }
        }
      `}</style>
      ...
      <div className="ll-grid" style={{ position: 'relative', zIndex: 1 }}>
        <div className="ll-photo">
          <img src={PHOTO_URL} alt="Kim Hoggan" />
          <div className="ll-fade-right" />
          <div className="ll-fade-bottom" />
        </div>
        ...
        <div className="ll-content">
          <div>
            ...
            <h1 style={{ fontFamily: "'DM Serif Display', serif", fontSize: 'clamp(2.4rem, 5vw, 4.8rem)', color: '#fffdf9', lineHeight: 1.05, marginBottom: '12px' }}>
              Kim <em style={{ fontStyle: 'italic', color: '#c69b63' }}>Hoggan</em>
            </h1>
            <p style={{ fontSize: '1.05rem', color: 'rgba(255,255,255,0.5)', fontWeight: 300, letterSpacing: '0.04em', marginBottom: '32px' }}>
